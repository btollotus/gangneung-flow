...
// 강릉FLOW 체크인 인증사진 1차 자동 검수.
// - 판단이 애매하거나 API 호출/파싱이 실패하면 절대 auto_approved로 처리하지 않고 flagged(관리자 검수)로 fail-safe
// - minors_at_risk가 조금이라도 감지되면 즉시 blocked
export async function moderatePhoto(
  ...
  try {
    ...
    if (parsed.verdict === 'violation') {
      ...
      // 안전 카테고리(나체/폭력/혐오/자해/미성년자) 위반이 아닌 경우(예: unrelated_ad 단독)는
      // 즉시 차단하지 않고 관리자 검수로 넘긴다 — 주제 무관 사진은 안전 문제가 아니라서 과잉 차단 방지
